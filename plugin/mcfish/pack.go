...
func init() {
	...
	engine.OnFullMatch("查看钓鱼规则", getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *zero.Ctx) {
		msg := "一款钓鱼模拟器\n----------指令----------\n" +
			...
			"3.附魔书信息:\n-> 诱钓 : 减少上钩时间. 均价:1000, 上钩概率:0.25%\n-> 海之眷顾 : 增加宝藏上钩概率. 均价:2500, 上钩概率:0.10%\n-> 耐久 : 减少耐久消耗几率. 均价:2000, 上钩概率:0.15%\n-> 经验修补 : 钓鱼后自动消耗余额修复耐久. 均价:2000, 上钩概率:0.08%\n" +
			...
			"9.杂项:\n-> 无装备的情况下,每人最多可以购买3次100块钱的鱼竿,商店每日会上架1木竿\n-> 默认状态钓鱼上钩概率为60%(理论值!!!)\n-> 附魔的鱼竿会因附魔变得昂贵,每个附魔最高3级\n-> 耐久附魔每级使钓鱼消耗耐久的几率降低,公式为(60 + 40/(等级+1))%\n-> 经验修补附魔在钓鱼后自动消耗余额修复耐久,每点耐久消耗2点余额\n-> 三叉戟不算鱼竿,修复时可直接满耐久\n" +
			"-> 鱼竿数量大于50的不能买东西;\n     鱼竿数量大于30的不能钓鱼;\n     每购/售10次鱼竿获得1层宝藏诅咒;\n     每购买20次物品将获得3次价格减半福利;\n     每钓鱼75次获得1本净化书;\n" +
...
func drawEquipInfoBlock(equipInfo equip, fontdata []byte) (image.Image, error) {
	...
	textDy += textH * 3
	canvas.DrawString(" 附魔: 诱钓"+enchantLevel[equipInfo.Induce]+"  海之眷顾"+enchantLevel[equipInfo.Favor]+"  耐久"+enchantLevel[equipInfo.Durability]+"  经验修补"+enchantLevel[equipInfo.ExpRepair], textDx, textDy)
	return canvas.Image(), nil
}
